...
from graphgps.loader.ptc import PTCDataset
...
from graphgps.transform.transforms import (pre_transform_in_memory,
                                           ...
                                           clip_graphs_to_size)
from graphgps.transform.gen_subgraph import generate_subgraph
...
def load_dataset_master(format, name, dataset_dir):
    ...
    if cfg.sub.type:
        print('gen_subgraph using {}'.format(cfg.sub.type))  
        pre_transform_in_memory(dataset, partial(generate_subgraph(cfg)))
...
def preformat_TUDataset(dataset_dir, name):
    ...
    Returns:
        PyG dataset object
    """
    if name in ['DD', 'NCI1', 'ENZYMES', 'PROTEINS', 'TRIANGLES','MUTAG','PTC']:
        func = None
    ...
    if name =='PTC':
        dataset = PTCDataset(dataset_dir, name, pre_transform=func)
    else:
        dataset = TUDataset(dataset_dir, name, pre_transform=func)
    cfg.dataset.num_features = dataset.num_features
    cfg.dataset.num_classes = dataset.num_classes
    return dataset
...
def preformat_ZINC(dataset_dir, name):
    ...
    dataset = join_dataset_splits(
        [ZINC(root=dataset_dir, subset=(name == 'subset'), split=split,pre_transform=ZINC_Graph_Add_Mol())
        for split in ['train', 'val', 'test']]
    )
    return dataset
