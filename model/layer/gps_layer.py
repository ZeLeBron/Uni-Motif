...
class GPSLayer(nn.Module):
    ...
    def __init__(self, dim_h,
                 ...
                 bigbird_cfg=None, log_attn_weights=False,add_gnn=False,add_attn=False,add_bias=False):
        super().__init__()
        ...
        self.act_fn_ff = nn.ReLU()  # Add ReLU activation function
        if self.layer_norm:
        ...
        if self.add_gnn:
            self.gnn_sub_encoder=nn.Sequential(nn.Linear(dim_h, dim_h),nn.ReLU())
            self.gnn_edge_encoder=nn.Sequential(nn.Linear(dim_h, dim_h),nn.ReLU())
        if self.add_attn:
            self.attn_sub_encoder=nn.Sequential(nn.Linear(dim_h, dim_h),nn.ReLU())
            self.attn_x_encoder=nn.Sequential(nn.Linear(dim_h, dim_h),nn.ReLU())
        if self.add_bias:
            self.bias_sub_encoder=nn.Sequential(nn.Linear(dim_h, dim_h),nn.ReLU())
            self.bias_encoder=nn.Sequential(nn.Linear(dim_h, num_heads),nn.ReLU())
    ...
    def forward(self, batch):
        ...
        if self.add_gnn:
            ...
            substructure_x = scatter(batch.x[row], col, reduce = 'mean', dim = 0)
            fragment_emb=self.gnn_sub_encoder(substructure_x)
            sub_embedding_pooled=scatter(fragment_emb[col], row, reduce = 'mean', dim = 0,dim_size=batch.num_nodes)
            edge_frag=sub_embedding_pooled[batch.edge_index[0]]+sub_embedding_pooled[batch.edge_index[1]]
            batch.edge_attr+=self.gnn_edge_encoder(edge_frag)
        ...
        if self.self_attn is not None:
            if self.add_attn:
                row, col = batch.fragments_edge_index
                substructure_x = scatter(batch.x[row], col, reduce = 'mean', dim = 0)
                fragment_emb=self.attn_sub_encoder(substructure_x)
                sub_embedding_pooled=scatter(fragment_emb[col], row, reduce = 'mean', dim = 0,dim_size=batch.num_nodes)
                h=h+self.attn_x_encoder(sub_embedding_pooled)
            if self.global_model_type == 'BiasedTransformer' and self.add_bias:
                row, col = batch.fragments_edge_index
                substructure_x = scatter(batch.x[row], col, reduce = 'mean', dim = 0)
                fragment_emb=self.bias_sub_encoder(substructure_x)
                sub_embedding_pooled=scatter(fragment_emb[col], row, reduce = 'mean', dim = 0,dim_size=batch.num_nodes)
                edge_frag=sub_embedding_pooled[batch.edge_index[0]]+sub_embedding_pooled[batch.edge_index[1]]
                edge_bias=to_dense_adj(batch.edge_index,batch.batch,edge_frag)
                edge_bias=self.bias_encoder(edge_bias)
                B,N,N,H=edge_bias.shape
                batch.attn_bias=edge_bias.reshape(B*H,N,N)
